    ...
    return (
        <div className="bg-white h-[350px] dark:bg-gray-700 rounded-lg p-6 shadow-lg relative hover:shadow-xl transition duration-300 transform hover:-translate-y-1">
            <img
                ...
                src={imageSrc}
                alt={`${name}'s picture`}
                onError={(e) => e.target.src = 'https://via.placeholder.com/150'} // Fallback to placeholder
            />
            ...
            <div className="flex justify-center mt-2">
                <span className="text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                        <svg
                            key={i}
                            className={`w-5 h-5 inline-block fill-current ${i < rating ? 'text-yellow-400' : 'text-gray-300'}`}
                            viewBox="0 0 20 20"
                        >
